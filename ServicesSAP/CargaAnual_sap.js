...
const loginSap = async (req,res) => {
    ...
    try {
        ...
        InsertarServiceSAP(cookies, res,añoCarga);
...
const InsertarServiceSAP = async (cookies, res, añoCarga) => {
    ...
    try {
        ...
        const cargasPendientes = await obtenerCargasPendientes(añoCarga);
        if(cargasPendientes.length==0){
            res.status(200).json({message: "No hay datos pendientes en el año ingresado"});
            return;
        }
        ...
        res.status(200).json({message: "Carga de datos exitosa del añoCarga "+añoCarga});
    } catch (error) {
        console.error("Error al obtener las cargas pendientes:", error);
    }      
};
const obtenerCargasPendientes = async (añoCarga,res) => {
    try {
        const cargasPendientes = await new Promise((resolve, reject) => {
            Tbl_ValoresAnual.getCargasAnualEstadoPendiente(añoCarga, (err, result) => {
                if (err) {
                    console.error(err);
                    reject(err);
                }
